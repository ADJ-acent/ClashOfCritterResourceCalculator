...
"Or aim for something" takes an amount and a resource and solves for the
pinballs, then feeds that into the pinball box so the rest of the page answers
what else you collect on the way. It is a bisection over `amountOf()` rather
than a formula, because the three kinds of resource behave differently:

* **Track rewards** (Catch Tatari, drinks, cards, x2, candy) arrive in lumps at
  fixed rungs and stop for good at reward 70, so they have a **ceiling**. Asking
  for 300 Catch Tatari says the track only pays 240 rather than inventing a
  number.
* **Event material** trickles from every launch as well, so it has **no**
  ceiling: 5,000 boards is simply 14,029 pinballs.
* **Pinballs** mean the number you get to **play**, not a net gain, because that
  is what a "use N pinballs" quest counts. Playing 20,000 costs 17,441 of your
  own, since the track hands the rest back.

### Pinballs from the machine itself

`MACHINE.pPinBack` / `perPinBack` in `machine.js` are boilerplate for the slots
that pay pinballs, a second feedback loop alongside the track's. They are **0**
today, which makes every path inert, and the file documents what to set once the
odds are measured. The average is modelled (the pile stretches by
`1 / (1 - returnRate())`); the **spread it adds is not**, so the ranges under
"Show the spread" will read tighter than the truth until that is done.
